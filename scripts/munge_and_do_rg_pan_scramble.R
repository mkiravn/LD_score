...
library(tidyverse)
# Read in codes
phenotypes <- read.table("data/simons_phenotypes_codes.tsv", header = TRUE, sep = "\t") %>% tail(80)
...
tSDS_dir <- "data/sds"
output_dir <- "data/results/rgscramb"
...
for (i in c(1:dim(phenotypes)[1])) {
    ...
    tSDS_file <- file.path(tSDS_dir,"50.info0.allchroms.pan.tSDS.tsv")
    # Generate output file paths
    output_file <- file.path(output_dir, paste0("scram50.tSDS.",modified_code))
